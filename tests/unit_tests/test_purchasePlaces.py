...
class TestPurshase:
    def test_should_return_error_message_when_club_try_to_redeem_more_points_than_available(
        self, client, testing_data
    ):
        """
    ...
    def test_club_points_should_be_deducted_after_redeemed(self, client, testing_data):
        ...
        expected_values = {
            "number_of_club_points_updated": int(club["points"])
            - (number_of_place_club_try_to_redeem * POINTS_PER_PLACE)
        }
        response = client.post(
            "/purchasePlaces",
            data={
    ...
    def test_places_should_be_deducted_from_the_competition_when_redeemed(
        self, client, testing_data
    ):
        """
        ...
        expected_values = {
            "number_of_place_in_competitions_updated": int(
                competition["numberOfPlaces"]
            )
            - number_of_place_club_try_to_redeem
        }
        ...
        assert (
            ...
            == expected_values["number_of_place_in_competitions_updated"]
        )

    def test_club_should_be_able_to_book_12_or_less_place_per_competition(
        self, client, testing_data
    ):
        """
        test issue#4 : club's should not be able to book more than 12 places per competitons
    ...
    def test_club_should_not_be_able_to_book_more_than_12_place_per_competition_at_once(
        self, client, testing_data
    ):
        """
        test issue#4 : club's should not be able to book more than 12 places per competitons
    ...
    def test_club_should_not_be_able_to_book_more_than_12_place_per_competition_in_multiple_time(
        self, client, testing_data
    ):
        """
        test issue#4 : club's should not be able to book more than 12 places per competitons
    ...
    def test_shoul_not_be_able_to_book_past_competitions(self, client, testing_data):
        ...
        competition = testing_data["competitions"][
            4
        ]  # this competition is in the past (1900)
        number_of_place_club_try_to_redeem = 1
        ...
        expected_message = (
            "This competition already took place. You can not book place for it. "
        )
        assert response.status_code == 400
        assert expected_message in response.data.decode("UTF-8")
